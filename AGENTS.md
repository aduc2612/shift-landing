...
- Mark intentional simplifications with a `ponytail:` comment naming the ceiling and upgrade path if applicable.

**Not lazy about:**
- Input validation at trust boundaries (especially Supabase inserts).
- Error handling that prevents data loss.
- Security (RLS policies, env secrets, XSS prevention).
- Accessibility (WCAG AA).
- Anything explicitly requested in PLAN.md.

**Testing expectations:**
- Non-trivial logic leaves ONE runnable check behind (assert-based demo or one small test file; no frameworks, no fixtures).
- Trivial one-liners need no test.
- Email form, Supabase integration, and blog generation always get end-to-end verification.

---

## Project Context

**The whole app is at `C:\Shift`**, but for most cases you only need:
- `C:\Shift\.planning\shift\MARKETING.md` — product messaging and content
- PLAN.md — implementation phases
- This file (AGENTS.md) — development philosophy and patterns

**Before each task:**
1. Check available marketing skills and use appropriate ones if needed.
